...
void CN105Climate::loop() {
  ...
  // As long as the connection is not successful, we do not launch ANY cycle/write (otherwise it short-circuits the
  // delay). We still continue to read/process the input in order to detect 0x7A/0x7B (connection success).
  ...
  if (can_talk_to_hp) {
    ...
    bool temp_changed = !std::isnan(current_room_temp) &&
                        (std::isnan(this->last_room_temp_) || fabsf(current_room_temp - this->last_room_temp_) >= 0.1f);
  ...
  switch (state_) {
    case DriverState::BOOT: {
      ...
      this->set_timeout("cn105_bootstrap_timeout", 120000, [this]() {
        if (state_ >= DriverState::CONNECTING)
          return;
