...
void CN105Climate::send_first_connection_packet() {
  if (this->is_uart_ready()) {
    ...
    this->write_packet(packet, CONNECT_LEN,
                       false);  // checkIsActive=false because it's the first packet and we don't have any reply yet
...
void CN105Climate::write_packet(uint8_t *packet, int length, bool checkIsActive) {
  if ((this->is_uart_ready()) && (this->is_heatpump_connection_active() || (!checkIsActive))) {
...
const char *CN105Climate::get_power_setting() {
  HPPower p =
      (this->wantedSettings.power != HPPower::UNKNOWN) ? this->wantedSettings.power : this->currentSettings.power;
  return hp_power_to_str(p);
}

const char *CN105Climate::get_vane_setting() {
  HPVaneMode v =
      (this->wantedSettings.vane != HPVaneMode::UNKNOWN) ? this->wantedSettings.vane : this->currentSettings.vane;
  return hp_vane_to_str(v);
}

const char *CN105Climate::get_wide_vane_setting() {
  HPWideVaneMode wv = (this->wantedSettings.wideVane != HPWideVaneMode::UNKNOWN) ? this->wantedSettings.wideVane
...
const char *CN105Climate::get_airflow_control_setting() {
  HPAirflowControl ac = (this->wantedRunStates.airflow_control != HPAirflowControl::UNKNOWN)
                            ? this->wantedRunStates.airflow_control
...
void CN105Climate::create_packet(uint8_t *packet) {
  ...
  if (this->wantedSettings.wideVane != HPWideVaneMode::UNKNOWN) {
    ...
    if (val_opt) {
      ...
      ESP_LOGW(TAG, "Ignoring invalid wideVane setting while building packet");
    }
  }
...
void CN105Climate::publish_wanted_settings_state_to_ha() {
  ...
  if ((this->wantedSettings.vane != HPVaneMode::UNKNOWN) ||
      (this->wantedSettings.wideVane != HPWideVaneMode::UNKNOWN)) {
...
void CN105Climate::publish_wanted_run_states_state_to_ha() {
  if (this->wantedRunStates.airflow_control != HPAirflowControl::UNKNOWN) {
    if (this->airflow_control_select_ != nullptr) {
      if (this->has_changed(this->airflow_control_select_->current_option(),
                            hp_airflow_control_to_str(this->wantedRunStates.airflow_control),
  ...
  if (this->wantedRunStates.air_purifier > -1) {
    if (this->air_purifier_switch_ != nullptr &&
        this->air_purifier_switch_->state != this->wantedRunStates.air_purifier) {
...
void CN105Climate::send_remote_temperature_packet() {
  ...
  ESP_LOGD(LOG_REMOTE_TEMP, "Sending remote temperature packet... -> %.1f%s", this->remoteTemperature_,
